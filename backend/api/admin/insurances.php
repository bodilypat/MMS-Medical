<?php
	...
	function validateInsuranceInput($data) {
		...
		if ($data['start_date'] > $data['end_date']) {
			return 'Start date cannot be after and date';
		}
		return true;
	}
	...
	/* Create insurance */
	function createInsurance($pdo, $data) {
		...
		try {
			...
			sendResponse(201,['message' => 'Insurance created successfully']);
		} catch (PDOException $e) {
			sendResponse(500, ['error' => 'Creation failed: ' . $e->getMessage()]);
		}
	}

	/* Update insurance */
	function updateInsurance($pdo, $data) {
	
		if (empty($data['insurance_id'])) {
			return sendResponse(400, ['message' => 'insurance_id is required']);
		}
		
		try {
			$stmt = $pdo->prepare("SELECT * FROM insurance WHERE insurance_id = :insurance_id");
			$stmt->execute(['insurance_id' => $data['insurance_id']]);
			$existing = $stmt->fetch(PDO::FETCH_ASSOC);

			if (!$existing) {
				return sendResponse(404, ['message' => 'Insurance not found']);
			}

			if (!empty($data['start_date']) && !empty($data['end_date']) && $data['start_date'] > $data['end_date']) {
				return sendResponse(400, ['message' => 'Start date cannot be after end date']);
				return;
			}
			...
			$stmt->execute([
				...
				sendResponse(200,['message' => 'Insurance updated successfully']);
			} catch (PDOException $e) {
				sendResponse(500, ['error' => 'Update failed: ' . $e->getMessage()]);
		}
	}

	/* Delete insurance */
	function deleteInsurance($pdo, $data) {
		...
		try {
			...
			$stmt->execute(['insurance_id' => $data['insurance_id']]);
			sendRespnse(200, ['message' => 'Insurance record deleted']);
		} catch (PDOException $e) {
			sendResponse(500, ['error' => 'Deletion failed: ' . $e->getMessage()]);
		}
	}
?>
