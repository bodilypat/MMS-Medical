<?php
	...
	$route->put('api/users/{id}', [$patientController,'update']);
	$router->delete('api/users/{id}', [$userController,'destroy']);
	...
	/* Appointment Routes */
	$router->get('a/api/appointments', [$appointmentController,'index']);
	...
	$router->get('/api/medical_record/{id}', [$medicalRecordController,'store']);
	...
	$router->get('/api/lab_tests/{id}', [$labTestController,'store']);
	...
	$router->get('/api/prescriptions', [$prescriptController, 'index']);
	...
	/* Auth Routes */
	$router->post('api/login', functiion () use ($pdo) {
